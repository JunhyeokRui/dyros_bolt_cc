...
class CustomA2C(nn.Module):
    ...
    def __init__(self, config, checkpoint_directory):
        ...
        self.n_output = self.config.getint("data", "n_output_actor")
        ...
        self.actor = nn.Sequential(
            nn.Linear(self.config.getint("data", "n_input_feature"), self.config.getint("model", "hidden_size1")),
            nn.Tanh(),
            nn.Linear(self.config.getint("model", "hidden_size1"), self.config.getint("model", "hidden_size2")),
            nn.Tanh(),
            nn.Linear(self.config.getint("model", "hidden_size2"), self.config.getint("model", "hidden_size3")),
            nn.Tanh(),
            nn.Linear(self.config.getint("model", "hidden_size3"), self.config.getint("data", "n_output_actor"))
            )
        self.critic = nn.Sequential(
            ...
            nn.Linear(self.config.getint("model", "hidden_size2"), self.config.getint("model", "hidden_size3")),
            nn.Tanh(),
            nn.Linear(self.config.getint("model", "hidden_size3"), self.config.getint("data", "n_output_critic"))  
        )
